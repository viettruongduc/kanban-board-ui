...
const TasksContainer = () => {
  ...
  const handleDragEnd = async (result) => {
    ...
    const taskDestination = tasks.filter(
      (task) => task[0] === destination.droppableId
    );
  ...
  useEffect(() => {
    if (data) {
      ...
      const inDevelopment = data?.filter(
        (task) => task.title === "inDevelopment"
      );
      ...
      inDevelopment.forEach((task) =>
        inDevelopmentObject.items.push(task.items)
      );
                        ...
                        {task[1].items.map((item, index) => (
                          ...
                          >
                            {(provided) => (
                              ...
                              >
                                <label className={`deviceLabel ${item.device}`}>
                                  {item.device}
                                </label>
